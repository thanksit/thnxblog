<?php
class AdminthnxpostController extends ModuleAdminController {

    public function __construct() {
        $this->table = 'thnxposts';
        $this->className = 'thnxpostsclass';
        $this->lang = true;
        $this->deleted = false;
    ...
    {
        parent::init();
        $this->_join = 'LEFT JOIN '._DB_PREFIX_.'thnxposts_shop sbp ON a.id_thnxposts=sbp.id_thnxposts && sbp.id_shop IN('.implode(',',Shop::getContextListShopID()).')';
        $this->_select = 'sbp.id_shop';
        $this->_defaultOrderBy = 'a.position';
        $this->_defaultOrderWay = 'DESC';
        $this->_where = ' AND a.post_type = "post" ';
        if (Shop::isFeatureActive() && Shop::getContext() != Shop::CONTEXT_SHOP)
        $this->_group = 'GROUP BY a.id_thnxposts';
    ...
    {
    	$id_thnxposts = Tools::getValue("id_thnxposts");
    	$audio_temp = '';
    	$video_temp = '';
    	$gallery_temp = '';
    	$gallery_temp_str = '';
    	$post_img_temp = '';
    	if(isset($id_thnxposts) && !empty($id_thnxposts)){
    		$thnxpostsclass = new thnxpostsclass($id_thnxposts);
    		if(isset($thnxpostsclass->audio) && !empty($thnxpostsclass->audio)){
    			$audio_temp = @explode(",",$thnxpostsclass->audio);
    		}
    		if(isset($thnxpostsclass->video) && !empty($thnxpostsclass->video)){
    			$video_temp = @explode(",",$thnxpostsclass->video);
    		}
    		if(isset($thnxpostsclass->gallery) && !empty($thnxpostsclass->gallery)){
    			$gallery_temp = @explode(",",$thnxpostsclass->gallery);
    			$gallery_temp_str = $thnxpostsclass->gallery;
    		}
    		if(isset($thnxpostsclass->post_img) && !empty($thnxpostsclass->post_img)){
    			$post_img_temp = '<img src="'.thnxblog_img_uri.$thnxpostsclass->post_img.'" height="110" width="auto"><br>';
    		}
    	}
        ...
        if(!($thnxpostsclass = $this->loadObject(true)))
            return;
        $this->setdefaultvalue($thnxpostsclass);
    ...
    public function setdefaultvalue($obj)
    {
		...
		if(isset($obj->id) && !empty($obj->id)){
			$this->fields_value['meta_tag'] = thnxpostsclass::GetPostTags($obj->id);;
		}else{
    ...
    public function processPosition()
    {
        ...
        else if(!Validate::isLoadedObject($object = new thnxpostsclass((int)Tools::getValue($this->identifier, Tools::getValue('id_thnxposts', 1)))))
        ...
        else
        {
            ...
            Tools::redirectAdmin(self::$currentIndex.'&'.$this->table.'Orderby=position&'.$this->table.'Orderway=asc&conf=5'.(($id_thnxposts = (int)Tools::getValue($this->identifier)) ? ('&'.$this->identifier.'='.$id_thnxposts) : '').'&token='.Tools::getAdminTokenLite('Adminthnxcategory'));
        }
    }
    public function ajaxProcessUpdatePositions()
    {
      $id_thnxposts = (int)(Tools::getValue('id'));
      ...
      if (is_array($positions))
        ...
        {
          $pos = explode('_', $value);
          if ((isset($pos[1]) && isset($pos[2])) && ($pos[2] == $id_thnxposts))
          {
            $position = $key + 1;
            break;
          }
      ...
      $thnxpostsclass = new thnxpostsclass($id_thnxposts);
      if (Validate::isLoadedObject($thnxpostsclass))
      {
        if (isset($position) && $thnxpostsclass->updatePosition($way, $position))
        {
          Hook::exec('action'.$this->className.'Update');
          die(true);
        }
        else
          die('{"hasError" : true, errors : "Can not update thnxpostsclass position"}');
      }
      else
        die('{"hasError" : true, "errors" : "This thnxpostsclass can not be loaded"}');
    }
}