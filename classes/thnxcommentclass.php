...
class thnxcommentclass extends ObjectModel
{
	public $id;
	public $id_parent;
	public $id_post;
	...
	public function add($autodate = true, $null_values = false)
	{
	    ...
	    $comment_approved = (int)Configuration::get(thnxblog::$thnxblogshortname."comment_approved");
	...
	public static function getTopPosition()
	{
	    $sql = 'SELECT MAX(`position`)
	            FROM `'._DB_PREFIX_.'thnx_comments`';
    ...
    public static function getComments($id_post = NULL)
    {
		if($id_post == NULL)
			return false;
		$sql = 'SELECT * FROM `'._DB_PREFIX_.'thnx_comments` xc  WHERE xc.`id_post` = '.$id_post.' AND xc.active = 1 ORDER BY xc.position DESC ';
