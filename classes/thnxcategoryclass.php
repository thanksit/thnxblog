...
class thnxcategoryclass extends ObjectModel
{
	public $id;
	public $id_thnxcategory;
	public $name;
	public $description;
	public $link_rewrite;
	public $category_img;
	...
	{
        Shop::addTableAssociation('thnxcategory', array('type' => 'shop'));
    ...
    public static function CategoryExists($id_category = NULL,$category_type = 'post')
    {
		if($id_category == NULL || $id_category == 0)
			return false;
		$sql = 'SELECT xc.`id_thnxcategory` FROM `'._DB_PREFIX_.'thnxcategory` xc WHERE xc.`category_type` = "'.($category_type ? $category_type : 'category').'" AND xc.active = 1 AND xc.`id_thnxcategory` = '.$id_category;
		$rslts = Db::getInstance()->getrow($sql);
			return (isset($rslts['id_thnxcategory']) && !empty($rslts['id_thnxcategory'])) ? true : false;
    }
    public function update($null_values = false)
    {
    	if (isset($_FILES['category_img']) && isset($_FILES['category_img']['tmp_name']) && !empty($_FILES['category_img']['tmp_name'])) {
    		$this->category_img = thnxblog::UploadMedia('category_img');
    	}
    ...
    public function updatePosition($way, $position)
    {
        if (!$res = Db::getInstance()->executeS('
            SELECT `id_thnxcategory`, `position`
            FROM `'._DB_PREFIX_.'thnxcategory`
            ORDER BY `position` ASC'
        ))
            return false;
        if(!empty($res))
        foreach($res as $thnxcategory)
            if((int)$thnxcategory['id_thnxcategory'] == (int)$this->id)
        $moved_thnxcategory = $thnxcategory;
        if(!isset($moved_thnxcategory) || !isset($position))
            return false;
        $queryx = ' UPDATE `'._DB_PREFIX_.'thnxcategory`
        SET `position`= `position` '.($way ? '- 1' : '+ 1').'
        WHERE `position`
        '.($way
        ? '> '.(int)$moved_thnxcategory['position'].' AND `position` <= '.(int)$position
        : '< '.(int)$moved_thnxcategory['position'].' AND `position` >= '.(int)$position.'
        ');
        $queryy = ' UPDATE `'._DB_PREFIX_.'thnxcategory`
        SET `position` = '.(int)$position.'
        WHERE `id_thnxcategory` = '.(int)$moved_thnxcategory['id_thnxcategory'];
    ...
    public static function get_the_title($id_category = NULL,$category_type = 'category')
    {
		...
		$sql = 'SELECT xc.`id_thnxcategory`,xcl.`name`,xcl.`link_rewrite` FROM `'._DB_PREFIX_.'thnxcategory` xc INNER JOIN `'._DB_PREFIX_.'thnxcategory_lang` xcl ON (xc.`id_thnxcategory` = xcl.`id_thnxcategory` AND xcl.`id_lang` = '.$id_lang.') INNER JOIN `'._DB_PREFIX_.'thnxcategory_shop` xcs ON (xc.`id_thnxcategory` = xcs.`id_thnxcategory` AND xcs.`id_shop` = '.$id_shop.') ';
    ...
    public static function get_the_id($rewrite = NULL,$category_type = 'category')
    {
		...
		$id_lang = (int)Context::getContext()->language->id;
		$id_shop = (int)Context::getContext()->shop->id;
		$sql = 'SELECT xc.`id_thnxcategory` FROM `'._DB_PREFIX_.'thnxcategory` xc INNER JOIN `'._DB_PREFIX_.'thnxcategory_lang` xcl ON (xc.`id_thnxcategory` = xcl.`id_thnxcategory` AND xcl.`id_lang` = '.$id_lang.') INNER JOIN `'._DB_PREFIX_.'thnxcategory_shop` xcs ON (xc.`id_thnxcategory` = xcs.`id_thnxcategory` AND xcs.`id_shop` = '.$id_shop.') ';
		$sql .= ' WHERE xc.`category_type` = "'.($category_type ? $category_type : 'category').'" AND xcl.`link_rewrite` = "'.$rewrite.'" ';
		$rslts = Db::getInstance()->getrow($sql);
			return isset($rslts['id_thnxcategory']) ? $rslts['id_thnxcategory'] : NULL;
    }
    public static function GetCategories($category_type = 'category',$category_group = NULL)
    {
       $id_lang = (int)Context::getContext()->language->id;
       $id_shop = (int)Context::getContext()->shop->id;
       $sql = 'SELECT * FROM `'._DB_PREFIX_.'thnxcategory` xc 
    ...
    public static function SerializeCategory($brief = true)
    {
    	...
    	if(isset($category) && !empty($category)){
    		...
    		foreach ($category as $categoryvalue) {
    			$results[$i]['id'] = $categoryvalue['id_thnxcategory'];
