...
class thnxpostsclass extends ObjectModel
{
	public $id;
	public $id_thnxposts;
	public $post_author;
    ...
    public function update($null_values = false)
    {
		...
		if(empty($this->post_img) && isset($this->id)){
			$thnxpostsclass = new thnxpostsclass($this->id);
			$this->post_img = $thnxpostsclass->post_img;
		}
    ...
    {
    	$gallery_temp = thnxblog::BulkUploadMedia("gallery_temp");
    ...
    public static function getsinglepath($id_post = NULL,$post_type = 'post'){
    	...
    	$blog_url = thnxblog::thnxBlogLink();
    ...
    public function updatePosition($way, $position)
    {
        if(!$res = Db::getInstance()->executeS('
            SELECT `id_thnxposts`, `position`
            FROM `'._DB_PREFIX_.'thnxposts`
            ORDER BY `position` ASC'
        ))
            return false;
        if(!empty($res))
        foreach($res as $thnxposts)
            if((int)$thnxposts['id_thnxposts'] == (int)$this->id)
        $moved_thnxposts = $thnxposts;
        if(!isset($moved_thnxposts) || !isset($position))
            return false;
        $queryx = ' UPDATE `'._DB_PREFIX_.'thnxposts`
        SET `position`= `position` '.($way ? '- 1' : '+ 1').'
        WHERE `position`
        '.($way
        ? '> '.(int)$moved_thnxposts['position'].' AND `position` <= '.(int)$position
        : '< '.(int)$moved_thnxposts['position'].' AND `position` >= '.(int)$position.'
        ');
        $queryy = ' UPDATE `'._DB_PREFIX_.'thnxposts`
        SET `position` = '.(int)$position.'
        WHERE `id_thnxposts` = '.(int)$moved_thnxposts['id_thnxposts'];
    ...
    public static function TagPostInsert($id_post = NULL,$category_ids = NULL,$tag = 'tag'){
    	if($id_post == NULL || $category_ids == NULL){
    		...
    		if(isset($category_ids) && !empty($category_ids)){
    			...
    			if(Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.'thnx_category_post`(`id_post`, `id_category`,`type`) VALUES '.$queryval)){
    				return true;
    			}else{
    				return false;
    			}
    ...
    public static function DeleteTagPost($id_post = NULL,$tag = 'tag'){
		...
		if(Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'thnx_category_post WHERE id_post = '.$id_post.' AND type = "'.$tag.'"')){
			return true;
		}else{
			return false;
		}
    ...
    public static function get_the_title($id_post = NULL,$post_type = 'post')
    {
       ...
       $sql = 'SELECT xc.`id_thnxposts`,xc.`category_default`,xcl.`post_title`,xcl.`link_rewrite` FROM `'._DB_PREFIX_.'thnxposts` xc 
		INNER JOIN `'._DB_PREFIX_.'thnxposts_lang` xcl ON (xc.`id_thnxposts` = xcl.`id_thnxposts` AND xcl.`id_lang` = '.$id_lang.')
		INNER JOIN `'._DB_PREFIX_.'thnxposts_shop` xcs ON (xc.`id_thnxposts` = xcs.`id_thnxposts` AND xcs.`id_shop` = '.$id_shop.')
		';
       $sql .= ' WHERE xc.`post_type` = "'.($post_type?$post_type:'post').'" AND xc.`id_thnxposts` = '.$id_post;
    ...
    public static function get_the_id($rewrite = NULL,$post_type = 'post')
    {
		...
		$id_lang = (int)Context::getContext()->language->id;
		$id_shop = (int)Context::getContext()->shop->id;
		$sql = 'SELECT xc.`id_thnxposts` FROM `'._DB_PREFIX_.'thnxposts` xc INNER JOIN `'._DB_PREFIX_.'thnxposts_lang` xcl ON (xc.`id_thnxposts` = xcl.`id_thnxposts` AND xcl.`id_lang` = '.$id_lang.') INNER JOIN `'._DB_PREFIX_.'thnxposts_shop` xcs ON (xc.`id_thnxposts` = xcs.`id_thnxposts` AND xcs.`id_shop` = '.$id_shop.') ';
		$sql .= ' WHERE xc.`post_type` = "'.($post_type ? $post_type : 'post').'" AND xcl.`link_rewrite` = "'.$rewrite.'" ';
		$rslts = Db::getInstance()->getrow($sql);
			return isset($rslts['id_thnxposts']) ? $rslts['id_thnxposts'] : NULL;
    }
    public static function PostExists($id_post = NULL,$post_type = 'post')
    {
		if($id_post == NULL || $id_post == 0)
			return false;
		$sql = 'SELECT xc.`id_thnxposts` FROM `'._DB_PREFIX_.'thnxposts` xc WHERE xc.`post_type` = "'.($post_type ? $post_type : 'post').'" AND xc.active = 1 AND xc.`id_thnxposts` = '.$id_post;
		$rslts = Db::getInstance()->getrow($sql);
			return (isset($rslts['id_thnxposts']) && !empty($rslts['id_thnxposts'])) ? true : false;
    }
    public static function get_the_rewrite($id = NULL,$post_type = 'post')
    {
		...
		$id_lang = (int)Context::getContext()->language->id;
		$id_shop = (int)Context::getContext()->shop->id;
		$sql = 'SELECT xcl.`link_rewrite` FROM `'._DB_PREFIX_.'thnxposts` xc INNER JOIN `'._DB_PREFIX_.'thnxposts_lang` xcl ON (xc.`id_thnxposts` = xcl.`id_thnxposts` AND xcl.`id_lang` = '.$id_lang.') INNER JOIN `'._DB_PREFIX_.'thnxposts_shop` xcs ON (xc.`id_thnxposts` = xcs.`id_thnxposts` AND xcs.`id_shop` = '.$id_shop.') ';
		$sql .= ' WHERE xc.`post_type` = "'.($post_type ? $post_type : 'post').'" AND xc.`id_thnxposts` = "'.$id.'" ';
		$rslts = Db::getInstance()->getrow($sql);
    ...
    public static function get_the_category($id_category = NULL,$category_type = 'category')
    {
		...
		$id_lang = (int)Context::getContext()->language->id;
		$id_shop = (int)Context::getContext()->shop->id;
       $sql = 'SELECT xc.`id_thnxcategory`,xcl.`name`,xcl.`link_rewrite` FROM `'._DB_PREFIX_.'thnxcategory` xc 
		INNER JOIN `'._DB_PREFIX_.'thnxcategory_lang` xcl ON (xc.`id_thnxcategory` = xcl.`id_thnxcategory` AND xcl.`id_lang` = '.$id_lang.')
		INNER JOIN `'._DB_PREFIX_.'thnxcategory_shop` xcs ON (xc.`id_thnxcategory` = xcs.`id_thnxcategory` AND xcs.`id_shop` = '.$id_shop.')
		';
       $sql .= ' WHERE xc.`category_type` = "'.($category_type ? $category_type : 'category').'" AND xc.`id_thnxcategory` = '.$id_category;
       $rslts = Db::getInstance()->getrow($sql);
       		return $rslts;
    }
    public static function TagInsert($tag = NULL)
    {
		...
		$id_lang = (int)Context::getContext()->language->id;
		$id_shop = (int)Context::getContext()->shop->id;
       $sql = 'SELECT xc.`id_thnxcategory` FROM `'._DB_PREFIX_.'thnxcategory` xc 
		INNER JOIN `'._DB_PREFIX_.'thnxcategory_lang` xcl ON (xc.`id_thnxcategory` = xcl.`id_thnxcategory` AND xcl.`id_lang` = '.$id_lang.')
		INNER JOIN `'._DB_PREFIX_.'thnxcategory_shop` xcs ON (xc.`id_thnxcategory` = xcs.`id_thnxcategory` AND xcs.`id_shop` = '.$id_shop.')
		';
       ...
       if(isset($rslts) && !empty($rslts)){
       		return $rslts['id_thnxcategory'];
       }else{
       		$tagobj = new thnxcategoryclass();
    ...
    public static function GetPostTags($id_post = NULL,$tag = 'tag')
    {
		...
		$id_lang = (int)Context::getContext()->language->id;
		$id_shop = (int)Context::getContext()->shop->id;
       $sql = 'SELECT xcl.`name` FROM `'._DB_PREFIX_.'thnx_category_post` xcp 
		INNER JOIN `'._DB_PREFIX_.'thnxcategory` xc ON (xcp.`id_category` = xc.`id_thnxcategory` AND xc.`category_type` = "'.$tag.'")
		INNER JOIN `'._DB_PREFIX_.'thnxcategory_lang` xcl ON (xcp.`id_category` = xcl.`id_thnxcategory` AND xcl.`id_lang` = '.$id_shop.')
		INNER JOIN `'._DB_PREFIX_.'thnxcategory_shop` xcs ON (xcp.`id_category` = xcs.`id_thnxcategory` AND xcs.`id_shop` = '.$id_shop.')
		';
    ...
    public static function GetPostTagsResults($id_post = NULL,$tag = 'tag')
    {
		...
		$id_lang = (int)Context::getContext()->language->id;
		$id_shop = (int)Context::getContext()->shop->id;
       $sql = 'SELECT xcp.`id_category`,xcl.`name`,xcl.`link_rewrite` FROM `'._DB_PREFIX_.'thnx_category_post` xcp 
		INNER JOIN `'._DB_PREFIX_.'thnxcategory` xc ON (xcp.`id_category` = xc.`id_thnxcategory` AND xc.`category_type` = "'.$tag.'")
		INNER JOIN `'._DB_PREFIX_.'thnxcategory_lang` xcl ON (xcp.`id_category` = xcl.`id_thnxcategory` AND xcl.`id_lang` = '.$id_shop.')
		INNER JOIN `'._DB_PREFIX_.'thnxcategory_shop` xcs ON (xcp.`id_category` = xcs.`id_thnxcategory` AND xcs.`id_shop` = '.$id_shop.')
		';
    ...
    public static function GetBlogTags($count = 10,$tag = 'tag')
    {
		...
		$sql .= ' ORDER BY xc.`id_thnxcategory` DESC ';
		$sql .= ' LIMIT '.(int)$count;
		$rslts = Db::getInstance()->executeS($sql);
        if(isset($rslts) && !empty($rslts)){
       		$i = 0;
    ...
    public static function GetCategoryPostsCount($category_default = NULL,$post_type = 'post'){
		$id_lang = (int)Context::getContext()->language->id;
		$id_shop = (int)Context::getContext()->shop->id;
		$sql = 'SELECT count(xc.`id_thnxposts`) as allthnxposts FROM `'._DB_PREFIX_.'thnxposts` xc 
		INNER JOIN `'._DB_PREFIX_.'thnxposts_lang` xcl ON (xc.`id_thnxposts` = xcl.`id_thnxposts` AND xcl.`id_lang` = '.$id_lang.')
		INNER JOIN `'._DB_PREFIX_.'thnxposts_shop` xcs ON (xc.`id_thnxposts` = xcs.`id_thnxposts` AND xcs.`id_shop` = '.$id_shop.')
		';
        ...
        return (int)$queryexec['allthnxposts'];
    }
    public static function GetCategoryPosts($category_default = NULL,$p = NULL,$n = NULL,$post_type = 'post',$order_by = 'DESC')
    {
    	if($p == NULL || $p < 1)
    		$p = 1;
    	if($n == NULL)
    		$n = (int)Configuration::get(thnxblog::$thnxblogshortname."post_per_page");
		$results = array();
		$id_lang = (int)Context::getContext()->language->id;
		$id_shop = (int)Context::getContext()->shop->id;
		$GetAllImageTypes = thnximagetypeclass::GetAllImageTypes();
		$sql = 'SELECT * FROM `'._DB_PREFIX_.'thnxposts` xc 
		INNER JOIN `'._DB_PREFIX_.'thnxposts_lang` xcl ON (xc.`id_thnxposts` = xcl.`id_thnxposts` AND xcl.`id_lang` = '.$id_lang.')
		INNER JOIN `'._DB_PREFIX_.'thnxposts_shop` xcs ON (xc.`id_thnxposts` = xcs.`id_thnxposts` AND xcs.`id_shop` = '.$id_shop.')
		';
		$sql .= ' WHERE xc.`active` = 1 ';
	       			...
	       			if($qkey == 'post_img'){
	       				if(isset($GetAllImageTypes) && !empty($GetAllImageTypes)){
					        foreach ($GetAllImageTypes as $imagetype){
					        	$results[$i]['post_img_'.$imagetype['name']] = thnxblog_img_uri.$imagetype['name'].'-'.$qvalue;
					        	if(!self::ImageExists($imagetype['name'].'-'.$qvalue)){
					        		$results[$i]['post_img_'.$imagetype['name']] =	thnxblog_img_uri.$imagetype['name'].'-noimage.jpg';
					        	}
					        }
          			...
          			if(isset($qlvalue['gallery']) && !empty($qlvalue['gallery'])){
          				...
          				if(isset($gallery_lists) && !empty($gallery_lists)){
          					...
          					foreach ($gallery_lists as $gall) {
          						$results[$i]['gallery_lists'][$ij]['main'] = thnxblog_img_uri.$gall;
			       				if(isset($GetAllImageTypes) && !empty($GetAllImageTypes)){
							        foreach ($GetAllImageTypes as $imagetype){
							        	$results[$i]['gallery_lists'][$ij][$imagetype['name']] = thnxblog_img_uri.$imagetype['name'].'-'.$gall;
							        	if(!self::ImageExists($imagetype['name'].'-'.$gall)){
							        		$results[$i]['gallery_lists'][$ij][$imagetype['name']] =	thnxblog_img_uri.$imagetype['name'].'-noimage.jpg';
							        	}
							        }
								}
          					$ij++;
    ...
    public static function GetRecentPosts($count = 4,$post_type = 'post',$order_by = 'DESC')
    {
		...
		$sql .= ' ORDER BY xc.`id_thnxposts` '.$order_by;
		$sql .= ' LIMIT '.(int)$count;
		$queryexec = Db::getInstance()->executeS($sql);
        if(isset($queryexec) && !empty($queryexec)){
      		$i = 0;
    ...
    public static function PostCountUpdate($id = NULL){
    	if($id == NULL || $id == 0)
    		return false;
	    $sql = 'UPDATE '._DB_PREFIX_.'thnxposts as xc SET xc.comment_count = (xc.comment_count+1) where xc.id_thnxposts = '.$id;
    ...
    public static function GetSinglePost($id_post = NULL,$post_type = 'post')
    {
       ...
       $sql = 'SELECT * FROM `'._DB_PREFIX_.'thnxposts` xc 
               ...
               INNER JOIN `'._DB_PREFIX_.'thnxposts_shop` xcs ON (xc.`id_thnxposts` = xcs.`id_thnxposts` AND xcs.`id_shop` = '.$id_shop.')
               ';
       $sql .= ' WHERE xc.`active` = 1 AND xc.post_type = "'.$post_type.'" AND xc.id_thnxposts = '.(int)$id_post;
       $queryexec = Db::getInstance()->getrow($sql);
       if(isset($queryexec) && !empty($queryexec)){
    ...
    public static function GetTagPosts($id_tag = NULL,$p = NULL,$n = NULL,$post_type = 'post',$order_by = 'DESC')
    {
		...
		INNER JOIN `'._DB_PREFIX_.'thnxposts_lang` xcl ON (xc.`id_thnxposts` = xcl.`id_thnxposts` AND xcl.`id_lang` = '.$id_lang.')
		INNER JOIN `'._DB_PREFIX_.'thnxposts_shop` xcs ON (xc.`id_thnxposts` = xcs.`id_thnxposts` AND xcs.`id_shop` = '.$id_shop.')
		INNER JOIN `'._DB_PREFIX_.'thnx_category_post` xcp ON (xcp.`id_post` = xc.`id_thnxposts` AND xcp.`id_category` = '.(int)$id_tag.' ) 
		';
		$sql .= ' WHERE xc.`active` = 1 ';
		if($post_type != NULL){
			$sql .= ' AND xc.post_type = "'.$post_type.'" ';
