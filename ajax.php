...
require_once (_PS_MODULE_DIR_.'thnxblog/thnxblog.php');
if(isset($_POST) && !empty($_POST)){
	$commentobj = new thnxcommentclass();
	$results = array();
	if(isset($_POST['name']) && !empty($_POST['name'])){
		$commentobj->name = $_POST['name'];
	}else{
		$results['error'][] = Context::getContext()->getTranslator()->trans("Name Is Required", array(), 'Modules.thnxblog.Admin');	
	}
	if(isset($_POST['email']) && !empty($_POST['email']) && Validate::isEmail($_POST['email'])){
		$commentobj->email = $_POST['email'];
	}else{
		$results['error'][] = Context::getContext()->getTranslator()->trans("Valid Email Address Is Required", array(), 'Modules.thnxblog.Admin');	
	}
    $commentobj->website = (isset($_POST['website']) && !empty($_POST['website'])) ? $_POST['website'] : '#';
    if(isset($_POST['subject']) && !empty($_POST['subject'])){
		$commentobj->subject = $_POST['subject'];
	}else{
		$results['error'][] = Context::getContext()->getTranslator()->trans("Subject Is Required", array(), 'Modules.thnxblog.Admin');
	}
	if(isset($_POST['content']) && !empty($_POST['content'])){
		$commentobj->content = $_POST['content'];
	}else{
		$results['error'][] = Context::getContext()->getTranslator()->trans("Comment Content Is Required", array(), 'Modules.thnxblog.Admin');
	}
	...
	if(isset($results['error']) && !empty($results['error'])){
		...
		$comment_approved = Configuration::get(thnxblog::$thnxblogshortname."comment_approved");
		...
		if($commentobj->add()){
			$url = thnxblog::thnxBlogPostLink(array('id' => $commentobj->id_post));
			...
			$results['success'][] = Context::getContext()->getTranslator()->trans("Successfully Comment Added", array(), 'Modules.thnxblog.Admin');
			$results['results'] = $commentobj;
			die(Tools::jsonEncode($results));
		}else{
			$results['error'][] = Context::getContext()->getTranslator()->trans("Something Wrong Please Try Again ! ", array(), 'Modules.thnxblog.Admin');
			die(Tools::jsonEncode($results));	
		}
	}
}